...
apiPatologia.listarExamenPatologia = (req, res) => {
    var desde=req.body.desde;
    var hasta=req.body.hasta;
    req.getConnection((err, conn) => {
        conn.query('call LISTA_EXAMEN_DE_PATOLOGIA(?,?)',[desde,hasta], (err, result,fields) => {
            if (err) { res.json(err) };
            res.json(result[0]);
            return;
        });
    ...
    req.getConnection((err, conn) => {
                conn.query('CALL MOD_UN_EXAMEN_DE_PATOLOGIA_PAMS(?,?,?,?,?,?,?,?) ',
                    ...
                    if (err) {
                        ...
                        res.json({ mensaje: 'Modificacion Exitosa' });
                        return;
